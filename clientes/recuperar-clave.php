...
<head>
  ...
  <meta name="description" content="Recuperación de contraseña - Portal de Clientes <?= BRAND_NAME ?>">
  <title><?= BRAND_NAME ?> | Recuperar Contraseña - Clientes</title>
    ...
    <div class="recovery-body">
      ...
      ?>
        <div class="alert alert-success" id="alertMessage">
          ...
          <span><strong>¡Éxito!</strong> Tus credenciales han sido enviadas al correo.</span>
      ...
      ?>
        <div class="alert alert-error" id="alertMessage">
          ...
          <span><strong>Error:</strong> No se encontró un usuario con ese correo.</span>
          <button type="button" class="alert-close" onclick="this.parentElement.style.display='none'">
            <i class="fas fa-times"></i>
          </button>
        </div>
      ...
      <div class="info-box">
        ...
        <p>Ingresa tu correo electrónico y te enviaremos tus credenciales de acceso. Revisa también tu carpeta de spam.</p>
      </div>

      <div style="text-align: center; margin-bottom: 32px;">
        <?php if(USE_ICON_AS_LOGO): ?>
          <div style="display: flex; align-items: center; justify-content: center; gap: 12px;">
            <i class="<?= BRAND_LOGO_ICON ?>" style="font-size: 36px; color: #48bb78;"></i>
            <h2 style="font-size: 32px; font-weight: 800; margin: 0; background: linear-gradient(135deg, #48bb78 0%, #38b2ac 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;"><?= BRAND_NAME ?></h2>
          </div>
        <?php else: ?>
  ...
  <script>
    ...
    recoveryForm.addEventListener('submit', async function(e) {
      ...
      try {
        ...
        if (result.success) {
          showAlert('success', result.message || 'Credenciales enviadas exitosamente.');
          emailInput.value = '';
        } else {
          showAlert('error', result.message || 'No se encontró un usuario con ese correo.');
        }
      } catch (error) {
