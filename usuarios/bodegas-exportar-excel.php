...
$hojaActiva->getStyle('A1:G1')->getFont()->setBold('Bold')->getColor()->setARGB(\PhpOffice\PhpSpreadsheet\Style\Color::COLOR_DARKBLUE);
...
$hojaActiva->setCellValue([5,1], 'ID. PRODUCTO');

$hojaActiva->getColumnDimension('F')->setWidth(80);
$hojaActiva->setCellValue([6,1], 'PRODUCTO');

$hojaActiva->getColumnDimension('G')->setWidth(20);
$hojaActiva->setCellValue([7,1], 'EXISTENCIAS');
...
while ($res = mysqli_fetch_array($consulta, MYSQLI_ASSOC)) {
    ...
    $hojaActiva->setCellValue('E'.$i, $res['prod_id']);
    $hojaActiva->setCellValue('F'.$i, $res['prod_nombre']);
    $hojaActiva->setCellValue('G'.$i, $res['prodb_existencias']);
	// Set cell A4 with a formula
	$hojaActiva->setCellValue(
		'H'.$i,
		'=IF(G'.$i.' < 0, "INGRESE EXISTENCIAS POSITIVAS, POR FAVOR.", "")'
	);
